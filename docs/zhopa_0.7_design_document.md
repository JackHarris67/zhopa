...
`zhopa.board_index` - runtime-only read-through views.
...
## 6. Runtime-only индекс мира: `zhopa.board_index`

### Назначение
Модуль держит runtime-only read-through views для ZHOPA: смарты, squads, topology, revisions.
...
### Что хранится
- save-safe task state;
- protected squad markers;
- surge lock markers;
- normalized loaded state.

Runtime-only, перестраивается из live engine/SIMBOARD data:
- entry snapshots;
- dirty flags;
- last seen time;
...
5. сохраняет только Z.H.O.P.A.-owned task/lock state; live identity/index data перестраивается из engine/SIMBOARD.
